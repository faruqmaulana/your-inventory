...
export default function AddExitItems({ data, session }) {
    ...
    useEffect(() => {
        const filteredData = uiState.item.filter((el) => { return el.id === state.goodsId })
    ...
    return (
        <div>
            <Button onClick={handleOpen} sx={{ bgcolor: 'primary.main', color: 'white', marginRight: 3, ":hover": { bgcolor: 'primary.dark' } }}>+ Input Barang Keluar</Button>
            <Modal
                    ...
                    <Box sx={style} className='modal-form'>
                        ...
                        <form onSubmit={handleSubmit}>
                            ...
                            <TextField
                                disabled={true}
                                required
                                value={`${isNaN(uiState.lastStock - parseInt(state.amount_out)) ? 0 : uiState.lastStock - parseInt(state.amount_out)}`}
                                onChange={handleChange}
                                name="stock"
                                style={{ marginBottom: 20 }}
                                fullWidth
